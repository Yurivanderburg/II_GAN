...
PATH_out = f"Data/Adhara/"
PATH_to_mask = f"Data/MultipleSources/masks/"
save_images = True
...
def main(name):
    ...
    PATH_out = f"Data/MultipleSources/"
    ...
    if sampling:
        ...
        sampling_mask = np.load(f"{PATH_to_mask}MASK_4_telescopes_{name}.npy")
    ...
    for filename in os.listdir(PATH_in):
        ...
        if save_images:
            if counter < 150: # Save to val
                cv2.imwrite(f"{PATH_out}val/{image_name}_{name}.jpg", combined_image)
            elif (counter >= 150) and (counter < 300):
                cv2.imwrite(f"{PATH_out}test/{image_name}_{name}.jpg", combined_image)
            else:
                cv2.imwrite(f"{PATH_out}train/{image_name}_{name}.jpg", combined_image)
