...
import time
from functions_LST import grids, ellip
...
def main():
    ...
    # Fixed parameters
    sx, sy, x, y = grids(1e-10, image_size, 4e-7)
    epsilon = 0.07
    ...
    # Altair: Range between 2.6 and 3.4 mas (~1.2e-8 and 1.6e-8 rad)
    rad_ = np.arange(1.2e-8, 1.6e-8, 1e-9)*0.5 # 5
    inclination_ = np.arange(0, 2*np.pi, np.pi/4)  # 8
    pa_ = np.arange(0, 2*np.pi, np.pi/4)  # 8
