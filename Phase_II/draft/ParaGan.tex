...
\begin{figure*}[htbp]
	...
	\begin{tikzpicture}[node distance=0.9cm]
		...
		% Data pipeline
		\node[block, fill=blue!15] (data) {Ground Truth \\ (Create $N$ Images of Stellar Objects)};
		...
		\node[block, fill=yellow!20, below=0.6cm of split] (val) {Validation Set \\ (Tune Hyperparameters)};
		\node[block, fill=orange!20, below right=0.6cm and 2.5cm of split] (test) {Testing Set \\ (Final Evaluation)};
		
		% Model training big box
		...
		% Generator side (left)
		\node[block, fill=cyan!15, below=1.2cm of mod, xshift=-3.5cm] (noise) {Random Noise $z$};
		\node[block, fill=green!20, below=of noise] (gen) {Generator $G(z)$};
		\node[block, fill=blue!10, below=of gen] (fake) {Generated Image};
		\node[block, fill=purple!20, below=of fake] (gloss) {Generator Loss \\ (Update $G$)};
		...
		\node[block, fill=red!20, below=of disc] (out) {Output: Real/Fake};
		...
		% GAN arrows
		\draw[arrow] (mod) -- (noise);
		\draw[arrow] (mod) -- (real);
		\draw[arrow] (noise) -- (gen);
		\draw[arrow] (gen) -- (fake);
		\draw[arrow] (fake.east) -- ++(0.5,0) |- (disc.west);
		\draw[arrow] (real) -- (disc);
		\draw[arrow] (disc) -- (out);
		
		% Feedback loops
		\draw[arrow] (out.south west) -- ++(-3,0) |- (gloss.east);
		\draw[arrow] (gloss.west) -- ++(-3,0) |- (gen.west);
		\draw[arrow] (out.south east) -- ++(3,0) |- (dloss.east);
		\draw[arrow] (dloss.east) -- ++(3,0) |- (disc.east);
	...
	\end{tikzpicture}
	\caption{The cGAN-based image reconstruction block diagram. Data is split into training (used for GAN optimization), validation (for hyperparameter tuning), and testing (for final evaluation). The GAN training loop optimizes Generator and Discriminator, while validation and testing feed into performance evaluation metrics.}
	\label{fig:gan_block}
\end{figure*}
