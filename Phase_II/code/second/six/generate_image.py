...
for r in rad:
    for i in inc:
        ...
        if (i == 0) or (i == np.pi):
           ...
           plt.colorbar(img) 
           plt.axis('off')
           plt.savefig(f'../result/ellipse/ellipse{pic}.jpg')
           plt.close()
           np.save(f'../result/ellipse_npy/ellipse{pic}.npy', ellipse)
           #plt.imsave('../result/ellipse/ellipse' + str(pic) + '.jpg', ellipse)
           #np.save('../result/ellip_npy/ellipse' + str(pic) + '.npy', ellipse)
           pic += 1
        else:
           for p in pa:
               for s in sq:
