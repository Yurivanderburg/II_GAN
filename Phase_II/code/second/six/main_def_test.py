...
plt.tight_layout()
plt.axis('off')
plt.savefig('../result/six/ft/ft.jpg')
plt.show()
...
plt.tight_layout()
plt.axis('off')
plt.savefig('../result/six/ft/ft_log.jpg')
plt.show()
plt.close()
...
plt.tight_layout()
plt.axis('off')
plt.savefig('../result/six/ft/ft_base.jpg')
plt.show()
plt.close()
...
plt.tight_layout()
plt.axis('off')
plt.savefig('../result/six/ft/ft_log_base.jpg')
plt.show()
plt.close()
...
sample_input, sample_real = fn.load('../result/six/test/ellipse18.jpg')                         # load the image to be trained in float32 tensor
print(np.shape(sample_input))
pic = 0
for i in [sample_input, sample_real]:
    plt.imshow(i)
    plt.savefig('../result/six/testing_image/sep' + str(pic) + '.png')
    pic += 1
    plt.show()
...
for i in [obs_jit, sky_jit]:
    plt.imshow(i)
    plt.savefig('../result/six/testing_image/jit' + str(pic) + '.png')
    pic += 1
    plt.show()
...
for i in range(4):                                                               # flip the images 4 times
    ...
    plt.subplot(2, 2, i+1)
    plt.imshow(jit_sky)
    plt.savefig('../result/six/testing_image/jit_sky' + str(pic) + '.png')
    plt.axis('off')
plt.show()
...
plt.imshow(spimage)
plt.savefig('../result/six/testing_image/salt_pepper.png')
plt.show()
