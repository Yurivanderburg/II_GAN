...
sample_input, sample_real = fn.load('../result/ellipse/ellipse10.jpg')                                  # load the image to be trained in float32 tensor

# Converts a Keras model to dot format and save to a file.
generator = mfn.Generator()
tf.keras.utils.plot_model(generator, to_file='../result/six/testing_image/generator.png', show_shapes=True, dpi=64) # structure of Generator according to the neuron
...
plt.savefig('../result/six/testing_image/gen.png')
plt.show()

discriminator = mfn.Discriminator()
tf.keras.utils.plot_model(discriminator, to_file='../result/six/testing_image/discriminator.png', show_shapes=True, dpi=64) # structure of Discriminator according to the neuron
...
plt.colorbar()
plt.savefig('../result/six/testing_image/disc.png')
plt.show()
...
train_dataset = tf.data.Dataset.list_files(str('../result/six/train/*.jpg'))                              # To create a dataset of all files matching a pattern
...
try:
    test_dataset = tf.data.Dataset.list_files(str('../result/six/test/*.jpg'))                            # to test the model
except tf.errors.InvalidArgumentError:
    test_dataset = tf.data.Dataset.list_files(str('../result/six/val/*.jpg'))                             # to test the validity
